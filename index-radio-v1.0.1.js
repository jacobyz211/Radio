...
function buildConfigPage(baseUrl) {
  ...
  h += 'p.sub{font-size:14px;color:#91a0b9;margin-bottom:20px;line-height:1.6}';
  ...
  h += '<div class="card">';
  h += '<div class="stat-grid"><div class="stat"><div class="stat-n">Live</div><div class="stat-l">Stations</div></div><div class="stat"><div class="stat-n">Global</div><div class="stat-l">Countries</div></div><div class="stat"><div class="stat-n">Free</div><div class="stat-l">No API Key</div></div></div>';
  h += '<p class="sub">Search and stream live radio stations inside Eclipse using Radio Browser. Browse by station name, country, language, genre, or trending stations.</p>';
  ...
  h += '<div class="warn">This addon uses public Radio Browser data and resolved stream URLs. Tokens survive server restarts when Redis is configured.</div></div>';
  h += '<div class="card"><h2>Import Stations to Eclipse Library</h2><p class="sub">Downloads a CSV you can import via Library → Import CSV in Eclipse. You can import a country, language, tag, or search term.</p>';
  ...
  h += '<div class="lbl">Value</div><input type="text" id="impValue" placeholder="Example: United States, english, jazz, or smooth jazz">';
  h += '<div class="hint">Examples: <code>country + United States</code>, <code>language + english</code>, <code>tag + jazz</code>, <code>search + lofi</code>, or <code>trending</code> with the value left blank.</div>';
  ...
  h += '<footer>Eclipse Radio Addon v1.0.1 • Powered by <a href="https://www.radio-browser.info/" target="_blank" rel="noopener noreferrer">Radio Browser</a> • <a href="' + baseUrl + '/health" target="_blank" rel="noopener noreferrer">Health</a></footer>';
  h += '<script>';
  h += 'var _gu="",_ru="";';
  h += 'function generate(){var btn=document.getElementById("genBtn"),st=document.getElementById("genStatus");btn.disabled=true;btn.textContent="Generating...";st.className="status spin";st.textContent="Creating your token…";fetch("/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}).then(function(r){return r.json();}).then(function(d){if(d.error){st.className="status err";st.textContent=d.error;btn.disabled=false;btn.textContent="Generate My Addon URL";return;}_gu=d.manifestUrl;document.getElementById("genUrl").textContent=_gu;document.getElementById("genBox").style.display="block";document.getElementById("impToken").value=_gu;st.className="status ok";st.textContent="✓ Your addon URL is ready";btn.disabled=false;btn.textContent="Regenerate URL";}).catch(function(e){st.className="status err";st.textContent="Error: "+e.message;btn.disabled=false;btn.textContent="Generate My Addon URL";});}';
  h += 'function copyUrl(){if(!_gu)return;navigator.clipboard.writeText(_gu).then(function(){var b=document.getElementById("copyBtn");b.textContent="Copied!";setTimeout(function(){b.textContent="Copy URL";},1500);});}';
  h += 'function doRefresh(){var btn=document.getElementById("refBtn"),eu=document.getElementById("existingUrl").value.trim();if(!eu){alert("Paste your existing addon URL first.");return;}btn.disabled=true;btn.textContent="Checking...";fetch("/refresh",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({existingUrl:eu})}).then(function(r){return r.json();}).then(function(d){if(d.error){alert(d.error);btn.disabled=false;btn.textContent="Restore Existing URL";return;}_ru=d.manifestUrl;document.getElementById("refUrl").textContent=_ru;document.getElementById("refBox").style.display="block";document.getElementById("impToken").value=_ru;btn.disabled=false;btn.textContent="Restore Again";}).catch(function(e){alert("Error: "+e.message);btn.disabled=false;btn.textContent="Restore Existing URL";});}';
  ...
  h += 'function doImport(){var btn=document.getElementById("impBtn"),raw=document.getElementById("impToken").value.trim(),type=document.getElementById("impType").value.trim(),value=document.getElementById("impValue").value.trim(),st=document.getElementById("impStatus"),pv=document.getElementById("impPreview");if(!raw){st.className="status err";st.textContent="Paste your addon URL first.";return;}if(!type){st.className="status err";st.textContent="Enter an import type.";return;}var tok=getTok(raw);if(!tok){st.className="status err";st.textContent="Could not find your token in the URL.";return;}btn.disabled=true;btn.textContent="Fetching...";st.className="status spin";st.textContent="Fetching stations...";pv.style.display="none";fetch("/u/"+tok+"/import?type="+encodeURIComponent(type)+"&value="+encodeURIComponent(value)).then(function(r){if(!r.ok){return r.json().then(function(e){throw new Error(e.error||("Server error "+r.status));});}return r.json();}).then(function(data){var tracks=data.tracks||[];if(!tracks.length)throw new Error("No stations found.");var rows=tracks.slice(0,60).map(function(t,i){return "<div class=\\"tr\\"><span class=\\"tn\\">"+(i+1)+"</span><div class=\\"ti\\"><div class=\\"tt\\">"+hesc(t.title)+"</div><div class=\\"ta\\">"+hesc(t.artist)+"</div></div></div>";}).join("");if(tracks.length>60)rows+="<div class=\\"tr\\" style=\\"text-align:center;color:#555\\">+"+(tracks.length-60)+" more stations</div>";pv.innerHTML=rows;pv.style.display="block";st.className="status ok";st.textContent="Found "+tracks.length+" stations in \""+data.title+"\"";var lines=["Title,Artist,Album,Duration"];tracks.forEach(function(t){function ce(s){s=String(s||"");if(s.indexOf(",")!==-1||s.indexOf("\"")!==-1)s="\""+s.replace(/\"/g,"\"\"")+"\"";return s;}lines.push(ce(t.title)+","+ce(t.artist)+","+ce(data.title||"")+","+ce(""));});var blob=new Blob([lines.join("\n")],{type:"text/csv"});var a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=(data.title||"radio").replace(/[^a-zA-Z0-9 _-]/g,"").trim()+".csv";document.body.appendChild(a);a.click();document.body.removeChild(a);btn.disabled=false;btn.textContent="Fetch & Download CSV";}).catch(function(e){st.className="status err";st.textContent=e.message;btn.disabled=false;btn.textContent="Fetch & Download CSV";});}';
  h += '</script></body></html>';
  return h;
}
