...
async function redisSave(token, entry) {
  ...
  try {
    await redis.set(
      'rb:token:' + token,
      JSON.stringify({ createdAt: entry.createdAt, lastUsed: entry.lastUsed, reqCount: entry.reqCount })
    );
  } catch (e) {
...
async function tokenMiddleware(req, res, next) {
  ...
  if (entry.reqCount % 20 === 0) await redisSave(req.params.token, entry);
  next();
}
...
function mapStationToTrack(station) {
  ...
  const obj = {
    ...
    streamURL: stream,
    format: detectFormat(stream, station.hls),
    isLive: true
  };
...
function mapCountryToPlaylist(c) {
  return {
    id: `rbcountry${encodeURIComponent(c.name)}`,
    title: `${cleanText(c.name)} Radio`,
    creator: 'Radio Browser',
    artworkURL: null,
    trackCount: parseInt(c.stationcount || 0, 10) || null,
...
function mapLanguageToPlaylist(l) {
  return {
    id: `rblang${encodeURIComponent(l.name)}`,
    title: `${cleanText(l.name)} Radio`,
    creator: 'Radio Browser',
    artworkURL: null,
    trackCount: parseInt(l.stationcount || 0, 10) || null,
...
function mapTagToPlaylist(t) {
  return {
    id: `rbtag${encodeURIComponent(t.name)}`,
    title: `${cleanText(t.name)} Radio`,
    creator: 'Radio Browser',
    artworkURL: null,
    trackCount: parseInt(t.stationcount || 0, 10) || null,
...
function scoreStation(station, q) {
  ...
  if (parseInt(station.clickcount || 0, 10) > 0) score += Math.min(parseInt(station.clickcount || 0, 10), 50);
  if (parseInt(station.votes || 0, 10) > 0) score += Math.min(parseInt(station.votes || 0, 10), 50);
...
.logo-wrap{display:flex;align-items:center;gap:14px;margin-bottom:28px}.logo-text{font-size:26px;font-weight:800;color:#56a7ff;letter-spacing:-.02em}.logo-sub{font-size:13px;color:#55657e;margin-top:3px}
...
h2{font-size:16px;font-weight:700;margin-bottom:14px;color:#fff}p.sub{font-size:14px;color:#91a0b9;margin-bottom:20px;line-height:1.6}
.stat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px}.stat{background:#0d1119;border:1px solid #1a2130;border-radius:10px;padding:14px;text-align:center}.stat-n{font-size:22px;font-weight:800;color:#56a7ff}.stat-l{font-size:11px;color:#667791;margin-top:3px}
.pills{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:24px}.pill{border-radius:20px;font-size:11px;font-weight:600;padding:4px 10px;background:#111a29;color:#56a7ff;border:1px solid #22324b}.pill.g{background:#111d15;color:#6ec77b;border-color:#26432c}
...
input{width:100%;background:#0c1018;border:1px solid #1e2634;border-radius:10px;color:#e7ebf3;font-size:14px;padding:12px 14px;margin-bottom:6px;outline:none;transition:border-color .15s}input:focus{border-color:#56a7ff}input::placeholder{color:#425066}
.hint{font-size:12px;color:#73829b;margin-bottom:12px;line-height:1.7}.hint code{background:#111826;padding:1px 5px;border-radius:4px;color:#7db8ff}
button{cursor:pointer;border:none;border-radius:10px;font-size:15px;font-weight:700;padding:13px;width:100%;margin-top:6px;margin-bottom:12px;transition:background .15s}.bo{background:#56a7ff;color:#07101e}.bo:hover{background:#7db8ff}.bo:disabled{background:#2a3547;color:#70809b;cursor:not-allowed}.bg{background:#15301b;color:#e7ebf3;border:1px solid #2c5a36}.bg:hover{background:#1a3d23}.bd{background:#171d28;color:#aab4c5;border:1px solid #283244;font-size:13px;padding:10px}.bd:hover{background:#1e2634;color:#fff}
.box{display:none;background:#0c1018;border:1px solid #1e2634;border-radius:12px;padding:18px;margin-bottom:14px}.blbl{font-size:10px;color:#6b7993;text-transform:uppercase;letter-spacing:.07em;margin-bottom:8px}.burl{font-size:12px;color:#7db8ff;word-break:break-all;font-family:SF Mono,ui-monospace,monospace;margin-bottom:14px;line-height:1.5}
...
.warn{background:#101726;border:1px solid #22324b;border-radius:10px;padding:14px;margin-top:20px;font-size:12px;color:#7fa9d9;line-height:1.7}.status{font-size:13px;color:#7a8aa5;margin:8px 0;min-height:18px}.status.ok{color:#6ec77b}.status.err{color:#d96b6b}.status.spin{color:#7db8ff}
.preview{background:#0c1018;border:1px solid #1a2230;border-radius:10px;padding:12px;max-height:240px;overflow-y:auto;margin-bottom:12px;display:none}.tr{display:flex;gap:10px;align-items:center;padding:6px 0;border-bottom:1px solid #151c29;font-size:13px}.tr:last-child{border-bottom:none}.tn{color:#4d5d78;font-size:11px;min-width:22px;text-align:right}.ti{flex:1;min-width:0}.tt{color:#e7ebf3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ta{color:#7e8ca3;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
footer{margin-top:32px;font-size:12px;color:#4e5c75;text-align:center;line-height:1.8}footer a{color:#4e5c75;text-decoration:none}
</style>
</head>
<body>
<div class="logo-wrap">
  <svg width="52" height="52" viewBox="0 0 52 52" fill="none">
    <circle cx="26" cy="26" r="26" fill="#13233b"></circle>
    <path d="M18 33c3-8 13-8 16 0" stroke="#56a7ff" stroke-width="3" stroke-linecap="round"></path>
    <path d="M14 27c5-13 19-13 24 0" stroke="#56a7ff" stroke-width="3" stroke-linecap="round" opacity=".7"></path>
    <circle cx="26" cy="36" r="2.5" fill="#56a7ff"></circle>
  </svg>
  <div>
    <div class="logo-text">Radio Browser</div>
    <div class="logo-sub">for Eclipse Music</div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="pills">
    <span class="pill">Live radio</span>
    <span class="pill">Countries</span>
    <span class="pill">Languages</span>
    <span class="pill">Genres</span>
    <span class="pill g">No signup needed</span>
    <span class="pill g">Direct streams</span>
  </div>

  <button type="button" class="bo" id="genBtn">Generate My Addon URL</button>

  <div class="box" id="genBox">
    <div class="blbl">Your addon URL — paste into Eclipse</div>
    <div class="burl" id="genUrl"></div>
    <button type="button" class="bd" id="copyBtn">Copy URL</button>
  </div>
  ...
  <button type="button" class="bg" id="refBtn">Restore Existing URL</button>

  <div class="box" id="refBox">
    <div class="blbl">Restored URL — still works in Eclipse</div>
    <div class="burl" id="refUrl"></div>
    <button type="button" class="bd" id="copyRefBtn">Copy URL</button>
  </div>
...
<div class="card">
  ...
  <div class="hint">Examples: <code>country</code> + <code>United States</code>, <code>language</code> + <code>english</code>, <code>tag</code> + <code>jazz</code>, <code>search</code> + <code>lofi</code>, or <code>trending</code> with the value left blank.</div>

  <div class="status" id="impStatus"></div>
  <div class="preview" id="impPreview"></div>
  <button type="button" class="bg" id="impBtn">Fetch & Download CSV</button>
</div>

<footer>
  Eclipse Radio Addon v1.0.1 · Powered by
...
function getTokenFromUrl(s) {
  const m = String(s || '').match(/\/u\/([a-f0-9]{28})\//i);
  return m ? m[1] : null;
}
...
async function generateUrl() {
  ...
  try {
    ...
    if (!r.ok || d.error) throw new Error(d.error || 'Failed to generate URL');

    generatedUrl = d.manifestUrl;
    document.getElementById('genUrl').textContent = generatedUrl;
    document.getElementById('genBox').style.display = 'block';
    document.getElementById('impToken').value = generatedUrl;
...
async function restoreUrl() {
  ...
  const input = document.getElementById('existingUrl').value.trim();
  if (!input) {
  ...
  try {
    const r = await fetch('/refresh', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ existingUrl: input })
    });
    const d = await r.json();
    if (!r.ok || d.error) throw new Error(d.error || 'Restore failed');

    restoredUrl = d.manifestUrl;
    document.getElementById('refUrl').textContent = restoredUrl;
    document.getElementById('refBox').style.display = 'block';
    document.getElementById('impToken').value = restoredUrl;
...
async function importCsv() {
  ...
  try {
    ...
    let rows = tracks.slice(0, 60).map((t, i) =>
      '<div class="tr"><span class="tn">' + (i + 1) + '</span><div class="ti"><div class="tt">' +
      escapeHtml(t.title) + '</div><div class="ta">' + escapeHtml(t.artist) + '</div></div></div>'
    ).join('');

    if (tracks.length > 60) rows += '<div class="tr" style="text-align:center;color:#555">+' + (tracks.length - 60) + ' more stations</div>';
    ...
    tracks.forEach(t => {
      lines.push([csvEscape(t.title), csvEscape(t.artist), csvEscape(data.title), ''].join(','));
    });

    const blob = new Blob([lines.join('\n')], { type: 'text/csv' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = (data.title || 'radio').replace(/[^a-zA-Z0-9 _-]/g, '').trim() + '.csv';
...
function copyText(value, buttonId) {
  if (!value) return;
...
document.getElementById('genBtn').addEventListener('click', generateUrl);
document.getElementById('copyBtn').addEventListener('click', () => copyText(generatedUrl, 'copyBtn'));
document.getElementById('refBtn').addEventListener('click', restoreUrl);
document.getElementById('copyRefBtn').addEventListener('click', () => copyText(restoredUrl, 'copyRefBtn'));
document.getElementById('impBtn').addEventListener('click', importCsv);
</script>
</body>
</html>`;
}
...
app.get('/u/:token/search', tokenMiddleware, async function(req, res) {
  ...
  if (!q) {
    return res.json({
      ...
      artists: [],
      playlists: [
        { id: 'rbtrending', title: 'Trending Radio', creator: 'Radio Browser', artworkURL: null, trackCount: 50, description: 'Stations trending right now' },
        { id: 'rbtopclicks', title: 'Popular Radio', creator: 'Radio Browser', artworkURL: null, trackCount: 50, description: 'Most clicked live stations' },
        { id: 'rbtopvoted', title: 'Top Voted Radio', creator: 'Radio Browser', artworkURL: null, trackCount: 50, description: 'Highest voted stations' }
      ]
    });
  }
  ...
  try {
    ...
    data.stations.forEach(s => {
      ...
      if (!artistMap.has(key)) {
        artistMap.set(key, {
          id: 'rbartist' + encodeURIComponent(country),
          name: country,
...
app.get('/u/:token/album/:id', tokenMiddleware, async function(req, res) {
  ...
  res.json({
    ...
    artworkURL: stationArtwork(station),
    year: null,
    description: [cleanText(station.tags), cleanText(station.country), cleanText(station.language), station.codec || '', station.bitrate ? `${station.bitrate}k` : ''].filter(Boolean).join(' • '),
    trackCount: 1,
    tracks: [track]
  });
});
...
app.get('/u/:token/artist/:id', tokenMiddleware, async function(req, res) {
  const rawId = req.params.id;
  const country = decodeURIComponent(rawId.replace(/^rbartist/, ''));
  try {
...
app.get('/u/:token/import', tokenMiddleware, async function(req, res) {
  ...
  const tracks = Array.isArray(rows)
    ? rows.filter(s => safeUrl(s.url_resolved || s.urlresolved || s.url) && s.lastcheckok === 1).slice(0, 100).map(mapStationToTrack)
    : [];
...
app.get('/health', function(req, res) {
  res.json({
    ...
    cachedPlaylists: PLAYLIST_CACHE.size,
    timestamp: new Date().toISOString()
  });
});

app.listen(PORT, () => {
  console.log(`Eclipse Radio Addon v1.0.1 on port ${PORT}`);
});
