// ============================================================
// Eclipse Radio Addon  v1.0.5
// Radio Browser + SomaFM + TuneIn
//
// CRITICAL RULE: Eclipse does NOT call /stream/{id} for tracks —
//   it ONLY uses the streamURL field from the search result.
//   If streamURL is null/missing the track is silently skipped.
//   Every track MUST have a valid streamURL.
//
// Stream ID prefixes:
//   rbst{uuid}    — Radio Browser  (streamURL from RB data)
//   soma_{id}     — SomaFM         (streamURL from channels.json)
//   tunein_{guid} — TuneIn         (streamURL resolved at search time)
// ============================================================
...
const UA                = 'EclipseRadioAddon/1.0.4';
...
// Normalise format strings to what Eclipse accepts:
// mp3 | aac | m4a | flac | wav | ogg | hls
function normaliseFormat(f) {
  const s = String(f || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  if (s === 'mp3')                                    return 'mp3';
  if (s === 'aac' || s === 'aacp' || s === 'mp3aacp' || s === 'aacplus') return 'aac';
  if (s === 'm4a')                                    return 'm4a';
  if (s === 'flac')                                   return 'flac';
  if (s === 'ogg' || s === 'opus' || s === 'vorbis')  return 'ogg';
...
function stationArtwork(station) {
  // Use real favicon if present; otherwise null — Eclipse shows its own placeholder
  return safeUrl(station.favicon) || null;
}
...
async function loadRbPlaylist(id) {
  ...
  if      (id.startsWith('rbcountry')) { const c = decodeURIComponent(id.replace('rbcountry', '')); rows = await rbGet('/json/stations/bycountrycodeexact/' + (c.toLowerCase() === 'united states' ? 'US' : encodeURIComponent(c)), { hidebroken: true, order: 'clickcount', reverse: true, limit: 50 }); if (!Array.isArray(rows) || !rows.length) rows = await rbGet('/json/stations/bycountryexact/' + encodeURIComponent(c), { hidebroken: true, order: 'clickcount', reverse: true, limit: 50 }); title = c + ' Radio'; description = 'Top live stations from ' + c; }
  else if (id.startsWith('rblang'))    { const l = decodeURIComponent(id.replace('rblang',    '')); rows = await rbGet('/json/stations/bylanguageexact/' + encodeURIComponent(l), { hidebroken: true, order: 'clickcount', reverse: true, limit: 50 }); title = l + ' Radio'; description = 'Top live stations in ' + l; }
  else if (id.startsWith('rbtag'))     { const t = decodeURIComponent(id.replace('rbtag',     '')); rows = await rbGet('/json/stations/bytagexact/' + encodeURIComponent(t), { hidebroken: true, order: 'clickcount', reverse: true, limit: 50 }); title = t + ' Radio'; description = t + ' radio stations'; }
  else if (id === 'rbtrending')  { rows = await rbGet('/json/stations', { hidebroken: true, order: 'clicktrend', reverse: true, limit: 50 }); title = 'Trending Radio';  description = 'Stations trending right now'; }
  else if (id === 'rbtopvoted')  { rows = await rbGet('/json/stations', { hidebroken: true, order: 'votes',      reverse: true, limit: 50 }); title = 'Top Voted Radio'; description = 'Highest voted stations'; }
  else if (id === 'rbtopclicks') { rows = await rbGet('/json/stations', { hidebroken: true, order: 'clickcount', reverse: true, limit: 50 }); title = 'Popular Radio';   description = 'Most clicked live stations'; }
  else if (id === 'rbus')        { rows = await rbGet('/json/stations/bycountrycodeexact/US', { hidebroken: true, order: 'clickcount', reverse: true, limit: 100 }); title = '🇺🇸 United States Radio'; description = 'Top US live radio'; }
  else { return null; }
...
// ══════════════════════════════════════════════════════════
// SOMAFM
// streamURL IS included at search time — Eclipse needs it.
// format is normalised so Eclipse never sees "aacp" etc.
// ══════════════════════════════════════════════════════════

async function fetchSomaChannels() {
  if (SOMA_CACHE.channels && Date.now() - SOMA_CACHE.ts < SOMA_TTL_MS) return SOMA_CACHE.channels;
  try {
    const r = await axios.get('https://api.somafm.com/channels.json', { headers: { 'User-Agent': UA }, timeout: 10000 });
    SOMA_CACHE.channels = Array.isArray(r.data.channels) ? r.data.channels : [];
    SOMA_CACHE.ts = Date.now();
    console.log('[SomaFM] Loaded', SOMA_CACHE.channels.length, 'channels');
    return SOMA_CACHE.channels;
  } catch (e) {
    console.warn('[SomaFM] fetch error:', e.message);
    return SOMA_CACHE.channels || [];
  }
}
...
function bestSomaStream(streams) {
  ...
  // Strictly prefer MP3 — most universally supported in iOS media players
...
function mapSomaChannel(ch) {
  ...
  const stream = best ? safeUrl(best.url) : safeUrl('https://ice2.somafm.com/' + ch.id + '-128-mp3');
  if (!stream) console.warn('[SomaFM] No usable stream URL for channel:', ch.id);
  return {
    id:         'soma_' + ch.id,
    ...
    // streamURL MUST be included — Eclipse won't play without it
    streamURL:  stream || safeUrl('https://ice2.somafm.com/' + ch.id + '-128-mp3'),
    format:     normaliseFormat(best ? best.format : 'mp3')
  };
}

function searchSoma(q) {
  const channels = SOMA_CACHE.channels || [];
  if (!channels.length) { console.warn('[SomaFM] Cache empty at searchSoma time'); return []; }
  const needle = cleanText(q).toLowerCase();
...
// ══════════════════════════════════════════════════════════
// TUNEIN
// Streams are resolved in parallel at search time so each
// track has a real streamURL before being sent to Eclipse.
// Stations that can't be resolved are dropped (not included).
// ══════════════════════════════════════════════════════════

async function parsePls(url) {
  try {
    const r = await axios.get(url, { headers: { 'User-Agent': UA }, timeout: 5000, responseType: 'text' });
...
// Resolve a TuneIn guide_id to a direct playable stream URL.
// Handles PLS/M3U unwrapping so Eclipse always gets a raw stream.
async function tuneInResolve(guideId) {
  try {
    ...
    for (const entry of body) {
      ...
      // Unwrap PLS → get real stream
      if (/\.pls(\?|$)/i.test(url)) {
        const real = await parsePls(url);
        if (real && safeUrl(real)) return { url: real, format: 'mp3', bitrate: entry.bitrate };
        continue;
      }
      // Unwrap M3U8 (HLS) — Eclipse can play HLS directly
      if (/\.m3u8(\?|$)/i.test(url)) return { url, format: 'hls', bitrate: entry.bitrate };
      // Unwrap M3U playlist
      if (/\.m3u(\?|$)/i.test(url)) {
      ...
      // Direct stream URL
      return { url, format: normaliseFormat(entry.media_type || 'mp3'), bitrate: entry.bitrate };
    }
    return null;
  } catch (e) { console.warn('[TuneIn] resolve error:', guideId, e.message); return null; }
}
...
function mapTuneInStation(item, stream) {
  // Never use genre_id (internal codes like "g61") for album — use subtext or fallback
  const album = cleanText(item.subtext && !/^g\d+$/.test(item.subtext) ? item.subtext : 'Live Radio');
  return {
    ...
    album,
    duration:   null,
    artworkURL: safeUrl(item.image) || null,
    // streamURL resolved ahead of time so Eclipse can actually play it
    streamURL:  stream ? stream.url : null,
    format:     stream ? stream.format : 'mp3'
  };
}

// ══════════════════════════════════════════════════════════
// UNIFIED SCORING
// Higher = shown first. Works across all three sources.
// ══════════════════════════════════════════════════════════
...
// Phase 2: parallel TuneIn stream resolution (top 6 only)
// ══════════════════════════════════════════════════════════
...
async function combinedSearch(q) {
  ...
  // Phase 1 — all three searches in parallel
  ...
  // Phase 2 — resolve TuneIn streams in parallel, capped at 6 to stay fast
  const tuneInTop      = tuneInItems.slice(0, 6);
  const tuneInResolved = await Promise.allSettled(tuneInTop.map(item => tuneInResolve(item.guide_id)));
  const tuneInTracks   = [];
  tuneInTop.forEach((item, i) => {
    const r = tuneInResolved[i];
    const stream = (r.status === 'fulfilled') ? r.value : null;
    if (stream && safeUrl(stream.url)) {
      tuneInTracks.push(mapTuneInStation(item, stream));
    }
    // Drop stations whose stream couldn't be resolved — Eclipse can't play them
  });
  ...
  for (const track of [...somaTracks, ...tuneInTracks, ...rbTracks]) {
    if (!track.title || !track.streamURL) continue; // drop anything without a playable URL
  ...
  return { tracks: merged.slice(0, 30), playlists: [somaPlaylist, ...rbPlaylists].slice(0, 15) };
}
...
async function loadPlaylist(id) {
  if (id === 'somaall') {
    ...
    const tracks   = channels.map(mapSomaChannel).filter(t => t.streamURL);
    const playlist = { id: 'somaall', title: 'SomaFM — All Channels', description: '30+ curated US internet radio channels', artworkURL: 'https://somafm.com/img3/logo-400.png', creator: 'SomaFM', tracks };
...
function buildConfigPage(baseUrl) {
  ...
  h += '</style></head><body>';
  h += '<div class="logo-wrap"><svg width="52" height="52" viewBox="0 0 52 52" fill="none"><circle cx="26" cy="26" r="26" fill="#13233b"></circle><path d="M18 33c3-8 13-8 16 0" stroke="#56a7ff" stroke-width="3" stroke-linecap="round"></path><path d="M14 27c5-13 19-13 24 0" stroke="#56a7ff" stroke-width="3" stroke-linecap="round" opacity=".7"></path><circle cx="26" cy="36" r="2.5" fill="#56a7ff"></circle></svg><div><div class="logo-text">Radio</div><div class="logo-sub">for Eclipse Music \u00b7 v1.0.5</div></div></div>';
  h += '<div class="card"><div class="stat-grid"><div class="stat"><div class="stat-n">3</div><div class="stat-l">Sources</div></div><div class="stat"><div class="stat-n">30k+</div><div class="stat-l">Stations</div></div><div class="stat"><div class="stat-n">Free</div><div class="stat-l">No API Key</div></div></div>';
  h += '<p class="sub">Live radio from Radio Browser, SomaFM, and TuneIn. Search by station name, frequency, genre, country, or language.</p>';
  ...
  h += '<div class="lbl">Restore an existing URL</div><input type="text" id="existingUrl" placeholder="Paste your existing addon URL here"><button class="bg" id="refBtn" onclick="doRefresh()">Restore Existing URL</button>';
  h += '<div class="box" id="refBox"><div class="blbl">Restored</div><div class="burl" id="refUrl"></div><button class="bd" id="copyRefBtn" onclick="copyRef()">Copy URL</button></div>';
  h += '<hr><div class="steps"><div class="step"><div class="sn">1</div><div class="st">Click <b>Generate My Addon URL</b></div></div><div class="step"><div class="sn">2</div><div class="st">Copy your URL</div></div><div class="step"><div class="sn">3</div><div class="st">Open <b>Eclipse</b> \u2192 Settings \u2192 Connections \u2192 Add Connection \u2192 Addon</div></div><div class="step"><div class="sn">4</div><div class="st">Paste your URL and tap <b>Install</b></div></div></div>';
  h += '<div class="warn">Your token is saved to Redis \u2014 it survives server restarts. Each person should generate their own URL.</div></div>';
  h += '<div class="card"><h2>Import Stations to Eclipse Library</h2><p class="sub">Downloads a CSV for Library \u2192 Import CSV in Eclipse.</p>';
  h += '<div class="lbl">Your Addon URL</div><input type="text" id="impToken" placeholder="Paste your addon URL \u2014 auto-fills after generating">';
  h += '<div class="lbl">Import type</div><input type="text" id="impType" placeholder="country, language, tag, trending, topvoted, topclicks, search, or somafm">';
  h += '<div class="lbl">Value</div><input type="text" id="impValue" placeholder="Example: United States, english, jazz \u2014 leave blank for somafm or trending">';
  ...
  h += '<div class="status" id="impStatus"></div><div class="preview" id="impPreview"></div><button class="bg" id="impBtn" onclick="doImport()">Fetch &amp; Download CSV</button></div>';
  h += '<footer>Eclipse Radio Addon v1.0.5 \u2022 <a href="https://www.radio-browser.info" target="_blank" rel="noopener noreferrer">Radio Browser</a> \u2022 <a href="https://somafm.com" target="_blank" rel="noopener noreferrer">SomaFM</a> \u2022 <a href="https://tunein.com" target="_blank" rel="noopener noreferrer">TuneIn</a> \u2022 <a href="' + baseUrl + '/health" target="_blank" rel="noopener noreferrer">Health</a></footer>';
  h += '<script>var gu="",ru="";function getTok(s){var m=String(s||"").match(/\\/u\\/([a-f0-9]{28})\\//i);return m?m[1]:null;}function hesc(s){return String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");}';
  h += 'function generate(){var btn=document.getElementById("genBtn"),st=document.getElementById("genStatus");btn.disabled=true;btn.textContent="Generating...";st.className="status spin";st.textContent="Creating your token\u2026";fetch("/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}).then(function(r){return r.json();}).then(function(d){if(d.error){st.className="status err";st.textContent=d.error;btn.disabled=false;btn.textContent="Generate My Addon URL";return;}gu=d.manifestUrl;document.getElementById("genUrl").textContent=gu;document.getElementById("genBox").style.display="block";document.getElementById("impToken").value=gu;st.className="status ok";st.textContent="\u2713 Your addon URL is ready";btn.disabled=false;btn.textContent="Generate My Addon URL";}).catch(function(e){st.className="status err";st.textContent="Network error";btn.disabled=false;btn.textContent="Generate My Addon URL";});}';
  ...
  h += 'function doRefresh(){var raw=document.getElementById("existingUrl").value.trim(),btn=document.getElementById("refBtn"),st=document.getElementById("genStatus");if(!raw)return;btn.disabled=true;btn.textContent="Restoring...";fetch("/refresh",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({existingUrl:raw})}).then(function(r){return r.json();}).then(function(d){if(d.error){st.className="status err";st.textContent=d.error;btn.disabled=false;btn.textContent="Restore Existing URL";return;}ru=d.manifestUrl;document.getElementById("refUrl").textContent=ru;document.getElementById("refBox").style.display="block";document.getElementById("impToken").value=ru;st.className="status ok";st.textContent="\u2713 URL restored";btn.disabled=false;btn.textContent="Restore Existing URL";}).catch(function(){btn.disabled=false;btn.textContent="Restore Existing URL";});}';
  ...
  h += 'function doImport(){var addonUrl=document.getElementById("impToken").value.trim();var type=document.getElementById("impType").value.trim().toLowerCase();var value=document.getElementById("impValue").value.trim();var st=document.getElementById("impStatus"),prev=document.getElementById("impPreview"),btn=document.getElementById("impBtn");if(!addonUrl||!type){st.className="status err";st.textContent="Fill in addon URL and type.";return;}var tok=getTok(addonUrl);if(!tok){st.className="status err";st.textContent="Paste your full addon URL.";return;}st.className="status spin";st.textContent="Fetching stations\u2026";btn.disabled=true;prev.style.display="none";prev.innerHTML="";var url="/u/"+tok+"/import?type="+encodeURIComponent(type)+"&value="+encodeURIComponent(value);fetch(url).then(function(r){return r.json();}).then(function(d){if(d.error){st.className="status err";st.textContent=d.error;btn.disabled=false;return;}var tracks=d.tracks||[];st.className="status ok";st.textContent="\u2713 "+tracks.length+" stations fetched";if(tracks.length){prev.style.display="block";var inner="";tracks.slice(0,20).forEach(function(t,i){inner+="<div class=\\"tr\\"><div class=\\"tn\\">"+(i+1)+"</div><div class=\\"ti\\"><div class=\\"tt\\">"+hesc(t.title)+"</div><div class=\\"ta\\">"+hesc(t.artist)+"</div></div></div>";});prev.innerHTML=inner;}var csv="title,artist,album,streamURL\\n"+tracks.map(function(t){return [t.title,t.artist,t.album||"",t.streamURL||""].map(function(v){return "\\""+String(v||"").replace(/"/g,"\\"\\"")+"\\""}).join(",");}).join("\\n");var blob=new Blob([csv],{type:"text/csv"});var a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=(d.title||type).replace(/[^a-z0-9]+/gi,"-")+".csv";a.click();btn.disabled=false;}).catch(function(e){st.className="status err";st.textContent="Fetch failed: "+e.message;btn.disabled=false;});}';
  h += '<\/script></body></html>';
  return h;
}
...
app.get('/u/:token/manifest.json', tokenMiddleware, function(req, res) {
  res.json({ id: 'com.eclipse.radio.' + req.params.token.slice(0, 8), name: 'Radio', version: '1.0.5', description: 'Live radio from Radio Browser, SomaFM & TuneIn. Search by name, frequency, genre, country, or language.', icon: getBaseUrl(req) + '/radio-icon.png?v=3', resources: ['search', 'stream', 'catalog'], types: ['track', 'album', 'artist', 'playlist'] });
});
...
app.get('/u/:token/search', tokenMiddleware, async function(req, res) {
  ...
  try {
    ...
    rbData.stations.forEach(s => { const c = cleanText(s.country || 'Unknown'); const k = c.toLowerCase(); if (!artistMap.has(k)) artistMap.set(k, { id: 'rbartist' + encodeURIComponent(c), name: c, artworkURL: stationArtwork(s), genres: [cleanText(s.language), cleanText(s.tags)].filter(Boolean).slice(0, 2) }); });
...
app.get('/u/:token/stream/:id', tokenMiddleware, async function(req, res) {
  ...
  if (id.startsWith('soma_')) {
    ...
    const best = bestSomaStream(ch.streams);
    if (!best || !safeUrl(best.url)) return res.status(404).json({ error: 'No stream URL for this channel.' });
  ...
  if (id.startsWith('tunein_')) {
    ...
    if (!stream || !safeUrl(stream.url)) return res.status(404).json({ error: 'TuneIn stream could not be resolved.' });
    return res.json(stream);
  }

  return res.status(404).json({ error: 'Unknown stream ID.', id });
});
...
app.get('/u/:token/album/:id', tokenMiddleware, async function(req, res) {
  ...
  if (id.startsWith('soma_')) {
    ...
    if (!ch) return res.status(404).json({ error: 'SomaFM channel not found.' });
    return res.json({ id, title: cleanText(ch.title), artist: 'SomaFM', artworkURL: safeUrl(ch.xlimage || ch.image) || 'https://somafm.com/img3/logo-400.png', year: null, description: cleanText(ch.description), trackCount: 1, tracks: [mapSomaChannel(ch)].filter(t => t.streamURL) });
  }
  const station = await stationByUuid(id.replace(/^rbst/, ''));
...
app.get('/u/:token/import', tokenMiddleware, async function(req, res) {
  ...
  if (type === 'somafm') {
    const channels = await fetchSomaChannels();
    return res.json({ title: 'SomaFM Radio', tracks: channels.map(mapSomaChannel).filter(t => t.streamURL) });
  }
  let title = 'Radio Import', rows = [];
  if      (type === 'country')   { title = value + ' Radio';    rows = await rbGet('/json/stations/bycountrycodeexact/' + (value.toLowerCase() === 'united states' ? 'US' : encodeURIComponent(value)), { hidebroken: true, order: 'clickcount', reverse: true, limit: 100 }); if (!Array.isArray(rows) || !rows.length) rows = await rbGet('/json/stations/bycountryexact/' + encodeURIComponent(value), { hidebroken: true, order: 'clickcount', reverse: true, limit: 100 }); }
  ...
  else if (type === 'search')    { const d = await searchStations(value); rows = d.stations; title = value + ' Radio'; }
...
app.get('/health', function(req, res) {
  res.json({ status: 'ok', version: '1.0.5', sources: ['radio-browser', 'somafm', 'tunein'], redisConnected: !!(redis && redis.status === 'ready'), somaChannelsCached: !!(SOMA_CACHE.channels && SOMA_CACHE.channels.length), somaChannelCount: (SOMA_CACHE.channels || []).length, activeTokens: TOKEN_CACHE.size, cachedStations: STATION_CACHE.size, cachedSearches: SEARCH_CACHE.size, timestamp: new Date().toISOString() });
});

app.listen(PORT, () => { console.log('Eclipse Radio Addon v1.0.5 on port ' + PORT); });
