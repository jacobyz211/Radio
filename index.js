...
function mapSomaChannel(ch) {
  ...
  const stream = best ? safeUrl(best.url) : null;
...
app.get('/u/:token/manifest.json', tokenMiddleware, function(req, res) {
  res.json({ id: 'com.eclipse.radio.' + req.params.token.slice(0, 8), name: 'Radio', version: '1.0.4', description: 'Live radio from Radio Browser, SomaFM & TuneIn. Search by name, frequency, genre, country, or language.', icon: 'https://somafm.com/img3/logo-400.png', resources: ['search', 'stream', 'catalog'], types: ['track', 'album', 'artist', 'playlist'] });
});
