...
async function redisSave(token, entry) {
  if (!redis) return;
  try {
    await redis.set('rb:token:' + token, JSON.stringify({
...
function mapStationToTrack(station) {
  ...
  const obj = {
    ...
    streamURL: safeUrl(station.url_resolved || station.urlresolved || station.url),
    format: detectFormat(station.url_resolved || station.urlresolved || station.url, station.hls),
    isLive: true
  };
...
function scoreStation(station, q) {
  ...
  const hay = [
    ...
    station.country,
    station.language
  ].map(x => cleanText(x).toLowerCase()).join(' ');
...
.burl{font-size:12px;color:#7db8ff;word-break:break-all;font-family:SF Mono,ui-monospace,monospace;margin-bottom:14px;line-height:1.5}
...
<div class="logo-wrap">
  <svg width="52" height="52" viewBox="0 0 52 52" fill="none">
    <circle cx="26" cy="26" r="26" fill="#13233b"/>
    <path d="M18 33c3-8 13-8 16 0" stroke="#56a7ff" stroke-width="3" stroke-linecap="round"/>
    <path d="M14 27c5-13 19-13 24 0" stroke="#56a7ff" stroke-width="3" stroke-linecap="round" opacity=".7"/>
    <circle cx="26" cy="36" r="2.5" fill="#56a7ff"/>
  </svg>
  <div>
...
<div class="card">
  ...
  <div class="box" id="genBox">
    ...
    <button class="bd" id="copyBtn" onclick="copyUrl()">Copy URL</button>
  </div>
  ...
  <button class="bg" id="refBtn" onclick="doRefresh()">Restore Existing URL</button>
...
function doImport() {
  var btn = document.getElementById('impBtn'),
      ...
      type = document.getElementById('impType').value.trim(),
      value = document.getElementById('impValue').value.trim(),
  ...
  fetch('/u/' + tok + '/import?type=' + encodeURIComponent(type) + '&value=' + encodeURIComponent(value))
    .then(function(r) {
      if (!r.ok) {
    ...
    .then(function(data) {
      ...
      tracks.forEach(function(t) {
        ...
        lines.push([ce(t.title), ce(t.artist), ce(data.title), ''].join(','));
      });

      var blob = new Blob([lines.join('\\n')], { type: 'text/csv' });
      var a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = data.title.replace(/[^a-zA-Z0-9 _-]/g, '').trim() + '.csv';
...
app.post('/refresh', async function(req, res) {
  ...
  if (!entry) {
    return res.status(404).json({ error: 'URL not found. Generate a new one.' });
  }
...
app.get('/u/:token/album/:id', tokenMiddleware, async function(req, res) {
  ...
  res.json({
    ...
    year: null,
    description: [
      cleanText(station.tags),
      cleanText(station.country),
      cleanText(station.language),
...
app.get('/u/:token/artist/:id', tokenMiddleware, async function(req, res) {
  ...
  try {
    const rows = await rbGet(`/json/stations/bycountryexact/${encodeURIComponent(country)}`, {
      hidebroken: true,
      order: 'clickcount',
      reverse: true,
      limit: 30
    ...
    const stations = Array.isArray(rows)
      ? rows.filter(s => safeUrl(s.url_resolved || s.urlresolved || s.url) && s.lastcheckok === 1)
      : [];
    ...
    res.json({
      ...
      genres: Array.from(new Set(
        stations.flatMap(s => cleanText(s.language).split(',').map(x => cleanText(x))).filter(Boolean)
      )).slice(0, 3),
...
app.get('/u/:token/import', tokenMiddleware, async function(req, res) {
  ...
  if (!type) {
    return res.status(400).json({ error: 'Pass ?type=country|language|tag|trending|topvoted|topclicks|search and optional value=' });
  }
  ...
  if (type === 'country') {
    title = `${value} Radio`;
    rows = await rbGet(`/json/stations/bycountryexact/${encodeURIComponent(value)}`, {
      hidebroken: true, order: 'clickcount', reverse: true, limit: 100
    });
  } else if (type === 'language') {
    title = `${value} Radio`;
    rows = await rbGet(`/json/stations/bylanguageexact/${encodeURIComponent(value)}`, {
      hidebroken: true, order: 'clickcount', reverse: true, limit: 100
    });
  } else if (type === 'tag') {
    title = `${value} Radio`;
    rows = await rbGet(`/json/stations/bytagexact/${encodeURIComponent(value)}`, {
      hidebroken: true, order: 'clickcount', reverse: true, limit: 100
    });
