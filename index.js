// ============================================================
// index.js — Eclipse Radio Addon v1.0.2
// Sources: Radio Browser + SomaFM + TuneIn
//
// Stream ID prefixes:
//   rbst{uuid}    — Radio Browser (streamURL included at search time — station URLs are stable)
//   soma_{id}     — SomaFM (streamURL included at search time — permanent CDN URLs, never expire)
//   tunein_{guid} — TuneIn (NO streamURL at search time — resolved fresh at play time via /stream)
// ============================================================
...
const STATION_CACHE    = new Map();  // rbst* Radio Browser stations
const SEARCH_CACHE     = new Map();  // RB search cache
const PLAYLIST_CACHE   = new Map();
const TUNEIN_META      = new Map();  // tunein_* station meta keyed by guide_id
...
const UA                = 'EclipseRadioAddon/1.0.2';
...
async function searchStations(q) {
  ...
  // Run global search + parallel US-scoped search simultaneously
  ...
  // Merge global + US results, deduplicate by stationuuid
  ...
  // For genre queries: also fetch stations directly by that tag when name results are thin
  ...
  // For language queries: also fetch stations directly by language
  ...
  const rankedStations = allStations
    ...
    .sort((a, b) => {
      ...
      // Frequency search (e.g. "95.5"): boost US stations so they surface first
...
async function loadRbPlaylist(id) {
  ...
  if (id.startsWith('rbcountry')) {
    ...
    // Use countrycode for US (Radio Browser stores it as "The United States Of America")
...
// ══════════════════════════════════════════════════════════
// SOMAFM
// SomaFM is a US-based internet radio broadcaster with 30+
// curated channels (jazz, lofi, electronic, ambient, etc).
// Their CDN stream URLs are permanent — safe to embed in
// search results as streamURL (Eclipse plays directly, no
// /stream call needed).
// ══════════════════════════════════════════════════════════
...
function bestSomaStream(streams) {
  ...
  // Prefer highest-quality MP3 for broadest device compatibility
  const mp3 = streams.filter(s => s.format === 'mp3').sort((a, b) => (b.quality === 'highest' ? 1 : 0) - (a.quality === 'highest' ? 1 : 0));
  if (mp3.length) return mp3[0];
  const aac = streams.filter(s => s.format === 'aac');
...
function mapSomaChannel(ch) {
  ...
  return {
    ...
    streamURL: best ? safeUrl(best.url) : null,  // Permanent CDN URL — embedded at search time
    format: best ? best.format : 'mp3'
  };
}
...
function searchSoma(q) {
  ...
  const needle = cleanText(q).toLowerCase();
  // "somafm" / "soma" / "soma fm" → return everything
...
// ══════════════════════════════════════════════════════════
// TUNEIN (unofficial opml.radiotime.com API)
// TuneIn has massive US AM/FM coverage — ESPN Radio, NPR,
// Fox News Radio, local stations, etc.
//
// WHY no streamURL at search time:
//   TuneIn returns redirect URLs (opml.radiotime.com/Tune.ashx)
//   that resolve to the actual stream. The underlying stream
//   URL can change at any time. By leaving streamURL out of
//   search results, Eclipse calls /stream/tunein_{guide_id}
//   right when the user taps play — that's when we hit
//   Tune.ashx to get the current live stream URL. Always fresh.
// ══════════════════════════════════════════════════════════
...
async function tuneInSearch(q) {
  try {
    ...
    // Only real audio stations, not topics/categories/shows
...
async function tuneInResolve(guideId) {
  try {
    const r = await axios.get(TUNEIN_BASE + '/Tune.ashx', {
      params: { id: guideId, render: 'json', formats: 'mp3,aac' },
      headers: { 'User-Agent': UA }, timeout: 10000
    });
    const body = r.data && r.data.body;
    if (!Array.isArray(body) || !body.length) return null;
    // Prefer a direct stream URL over playlist files
    const direct = body.find(s => safeUrl(s.url) && !/\.(pls|m3u|xspf)$/i.test(String(s.url)));
    const entry  = direct || body[0];
    if (!safeUrl(entry.url)) return null;
    return { url: entry.url, format: (entry.media_type || 'mp3').toLowerCase(), quality: entry.bitrate ? entry.bitrate + 'kbps' : undefined };
...
function mapTuneInStation(item) {
  ...
  return {
    ...
    // ← No streamURL here. Eclipse will call /stream/tunein_{guide_id} at play time.
    format:     item.formats ? item.formats.split(',')[0].trim() : 'mp3'
  };
}
...
async function combinedSearch(q) {
  ...
  // Warm SomaFM cache and run all three searches in parallel
  ...
  // Merge — SomaFM first (curated US), then TuneIn (US AM/FM), then Radio Browser (global)
  // Deduplicate by normalised title to avoid the same station appearing from two sources
  const seen   = new Set();
  const merged = [];
  ...
  // Build playlists from RB metadata
  const rbPlaylists = [
  ...
  // Add US playlist when the query is explicitly US-related
  ...
  const somaPlaylist = {
    id: 'somaall', title: 'SomaFM — All Channels', creator: 'SomaFM',
    artworkURL: 'https://somafm.com/img3/logo-400.png',
    trackCount: (SOMA_CACHE.channels || []).length || 30
  };
...
function buildConfigPage(baseUrl) {
  ...
  h += '<div><div class="logo-text">Radio</div><div class="logo-sub">for Eclipse Music · v1.0.2</div></div>';
  h += '</div>';
  ...
  h += '</div>';
  h += '<p class="sub">Live radio from Radio Browser, SomaFM, and TuneIn — all in one Eclipse addon. Search by station name, frequency, genre, country, or language.</p>';
  h += '<div class="pills">';
  h += '<span class="pill">Radio Browser</span><span class="pill o">SomaFM</span><span class="pill o">TuneIn</span>';
  h += '<span class="pill">US Stations</span><span class="pill">Genres</span><span class="pill g">No signup</span><span class="pill g">Direct streams</span>';
  h += '</div>';
  ...
  h += '<footer>Eclipse Radio Addon v1.0.2 &bull; <a href="https://www.radio-browser.info" target="_blank" rel="noopener noreferrer">Radio Browser</a> &bull; <a href="https://somafm.com" target="_blank" rel="noopener noreferrer">SomaFM</a> &bull; <a href="https://tunein.com" target="_blank" rel="noopener noreferrer">TuneIn</a> &bull; <a href="' + baseUrl + '/health" target="_blank" rel="noopener noreferrer">Health</a></footer>';
...
app.get('/u/:token/search', tokenMiddleware, async function(req, res) {
  ...
  // Empty query — return starter playlists (SomaFM + RB featured)
  if (!q) {
    // Pre-warm SomaFM cache in background so first real search is fast
  ...
  try {
    ...
    // Build albums + artists from Radio Browser stations only
...
app.get('/u/:token/stream/:id', tokenMiddleware, async function(req, res) {
  ...
  // ── SomaFM ─────────────────────────────────────────────
  // streamURL is already embedded in search results, but handle /stream
  // as a fallback (e.g. if Eclipse calls it anyway, or for offline caching)
  if (id.startsWith('soma_')) {
    ...
    return res.json({ url: best.url, format: best.format, quality: 'highest' });
  }

  // ── TuneIn ─────────────────────────────────────────────
  // Called at play time — we resolve the actual stream URL fresh right now.
  // This is intentional: TuneIn's Tune.ashx redirects can point to different
  // CDN URLs over time, so resolving here guarantees the URL is always current.
...
app.get('/u/:token/album/:id', tokenMiddleware, async function(req, res) {
  ...
  if (id.startsWith('soma_')) {
    ...
    const ch        = channels.find(c => c.id === channelId);
    if (!ch) return res.status(404).json({ error: 'SomaFM channel not found.' });
    const track = mapSomaChannel(ch);
    return res.json({ id, title: cleanText(ch.title), artist: 'SomaFM', artworkURL: track.artworkURL, year: null, description: cleanText(ch.description), trackCount: 1, tracks: [track] });
  }
  const uuid    = id.replace(/^rbst/, '');
  const station = await stationByUuid(uuid);
  if (!station) return res.status(404).json({ error: 'Station not found.' });
  const track = mapStationToTrack(station);
  res.json({ id, title: cleanText(station.name || 'Radio Station'), artist: cleanText(station.country || station.language || 'Radio Browser'), artworkURL: stationArtwork(station), year: null, description: stationSubtitle(station), trackCount: 1, tracks: [track] });
});
...
app.get('/u/:token/import', tokenMiddleware, async function(req, res) {
  ...
  if (type === 'somafm') {
    ...
    title  = 'SomaFM Radio';
    rows   = channels;
    return res.json({ title, tracks: rows.map(mapSomaChannel) });
  }
