...
<body>
  ...
  <script>
    ...
    async function doRefresh() {
      ...
      if (!eu) return alert('Paste your existing addon URL first.');
      btn.disabled = true;
    ...
    async function doImport() {
      ...
      try {
        ...
        let rows = tracks.slice(0, 60).map((t, i) => '<div class="tr"><span class="tn">' + (i + 1) + '</span><div class="ti"><div class="tt">' + hesc(t.title) + '</div><div class="ta">' + hesc(t.artist) + '</div></div></div>').join('');
        ...
        tracks.forEach(t => {
          ...
          lines.push(ce(t.title) + ',' + ce(t.artist) + ',' + ce(data.title || '') + ',' + ce(''));
        });
...
app.get('/health', function(req, res) {
  res.json({ status: 'ok', version: '1.0.1', radioBrowserBase: RB_BASE, redisConnected: !!(redis && redis.status === 'ready'), activeTokens: TOKEN_CACHE.size, cachedStations: STATION_CACHE.size, cachedSearches: SEARCH_CACHE.size, cachedPlaylists: PLAYLIST_CACHE.size, timestamp: new Date().toISOString() });
});
